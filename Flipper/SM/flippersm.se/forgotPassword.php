  ...
  function sendResetEmail($dbh, $player) {
    if (validEmail($player->mailAddress)) {
      ...
      $msg = 'Hello!
        ...
        You (or someone) has requested your password at FlipperSM.se to be reset. If you are not aware of this, you can safely ignore this message.
        ...
        If you encounter any problems, email us at support@flippersm.se for assistance.
        
        Regards
        /EPC 2013 organizers
      ';
      mail($player->mailAddress, 'FlipperSM.se password reset', $msg, 'From: support@flippersm.se');
      return true;
    } else {
      return false;
    }
  ...
  if ($player) {
    ...
    if ($nonce) {
      ...
      if ($player) {
        ...
        $content .= 'We could not not identify you, or you have already used this identity code. Please try again or <a href="mailto:support@flippersm.se">email us</a> for assistance.</p>';
      }
    } else {
        ...
        if ($player->mailAddress) {
          if (validEmail($player->mailAddress)) {
            if (sendResetEmail($dbh, $player)) {
              ...
              $content .= '<p>Something went wrong trying to send you a password reset email. Please try again or <a href="mailto:support@flippersm.se">email us</a> for assistance.</p>';
            }
          } else {
            $content .= 'There\'s something wrong with the email address registered for you. Please try again or <a href="mailto:support@flippersm.se">email us</a> for assistance.</p>';
          }
        } else {
          $content .= 'You have no email address registered with us. Please try again or <a href="mailto:support@flippersm.se">email us</a> for assistance.</p>';
        }
      } else {
        $content .= 'We could not not identify you. Please try again or <a href="mailto:support@flippersm.se">email us</a> for assistance.</p>';
      }
    } else if (!$success) {
      $content .= '
