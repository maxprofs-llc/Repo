...
<br><table class="tabell">
  <tr><td> Position: </td><td> Spelar-<br>nummer: </td><td> Tag: </td><td> Namn: </td><td> Hemort: </td><td> Kvalpoäng: </td><td> Antal reggade<br>kvalomg: </td></tr>
...
$sql = "SELECT IF(@a, @a:=@a+1, @a:=1) as Position, Players.Tag, Players.Name, Players.City, Players.Player_no, SUM(Qual_points) AS Qual_sum, COUNT(Players.Player_no) AS Kvalantal FROM Qual LEFT JOIN Players ON Players.Player_no = Qual.Player_no GROUP BY Name ORDER BY Qual_sum DESC";
...
for($c=0; $c<$intRows; $c++)
    ...
    printf("<td> <a href=\"sm07.showentry.php?no=%s\">%s</a> </td>", MySQL_result($sqlResult,$c,"Player_no"),MySQL_result($sqlResult,$c,$arySort["tag"]));
    ...
    printf("<td align=\"center\"> %s </td>", MySQL_result($sqlResult,$c,"Kvalantal"));
    echo "</tr>\n";
