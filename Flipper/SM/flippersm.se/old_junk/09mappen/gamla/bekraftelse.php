...
// input data
    ...
    $booAdminMail = mail("furir@flippersm.se, webb@flippersm.se", "SM-anmälan - $strTag", "Hejsan! Nu har det trillat in ännu en SM-anmälan från hemsidan.\n\nTag: $strTag\nNamn: $strFirstname $strLastname\nFrån: $strAddress, $strCity\nTelefon: $strPhone\nEmail: $strEmail\nÅlder: $intAge\nKval: $intQualG1, $intQualG2, $intQualG3, $intQualG4, $intQualG5\nTröjor: '$strOrder'\nPris: $intPrice kr\nIP: $strIP\nDatum: $strDate $strTime\nEventuella kommentarer: $strMessage\n\n\nHej då!\n", "From: SM 2007 <furir@flippersm.se>");
    $booPlayerMail = mail($strEmail, "Anmälan Flipper-SM 2007", "Hejsan!\n\nDu har nu anmält dig till de Svenska mästerskapen i flipperspel, 9-11 november 2007!\n\nGlöm inte att betala avgiften, som i ditt fall är $intPrice kronor, till följande konto hos Swedbank senast den 31 oktober:\nClearingnummer: 8304-8\nKontonummer: 3170013-1\nKontoinnehavare: Markus Salo\nSpar en kopia på din inbetalning. Det är viktigt att du anger namn och TAG för personen som betalningen avser.\n\nOBS! Det är de första 200 personerna som BÅDE anmäler sig och betalar som får plats! För att se om din betalning registrerats, kan du kolla på www.flippersm.se/index.php?sida=anmalda. Eftersom detta sker manuellt, kan det ta ett par dar innan din betalning syns på hemsidan.\n\nFöljande information gavs i anmälan:\n\nTag: $strTag\nNamn: $strFirstname $strLastname\nFrån: $strAddress, $strCity\nTelefon: $strPhone\nEmail: $strEmail\nÅlder: $intAge\nÖnskade kvaltider: Fre em-$intQualG1, Fre kv-$intQualG2, Lör fm-$intQualG3, Lör em-$intQualG4, Lör kv-$intQualG5\nTröjbeställning: '$strShirts'\nAvgift: $intPris\nDatum och tid: $strDate $strTime\nEventuellt meddelande: $strMessage\n\n\nOm nåt inte stämmer eller om du har andra frågor om din anmälan, skicka ett mail till Per på webb@flippersm.se. För allmänna frågor om Flipper-SM 2007, kontakta Markus på general@flippersm.se.\n\n\nHej då och lycka till!\n", "From: Flipper-SM 2007 <webb@flippersm.se>");
    
    
    if($intRows == 1 && $booAdminMail && $booPlayerMail)
        {
        echo "<h1>Anmälan - Flipper-SM</h1><br/>\n\n";
        
        echo "<p><b>Anmälan lyckades!</b><br/><br/>\n\n";
        ...
        echo "<b>OBS!</b> Det är de första 200 personerna som BÅDE anmäler sig och betalar som får plats! För att se om din betalning registrerats, kan du kolla på <a href='index.php?sida=anmalda'>anmälda-sidan</a>. Eftersom detta sker manuellt, kan det ta ett par dar innan din betalning syns på hemsidan.<br/><br/>\n\n";
        ...
        echo "<p>Anmälan lyckades <b>tyvärr inte</b>! Spamskyddet var ok, så du är förmodligen en människa, men databasen kunde inte nås. Var vänlig att gå tillbaka och försök igen.<br/><br/>\n\n";
        echo "Om problemet inte försvinner, kontakta <a href='mailto:webb@flippersm.se'>webb-ansvarige</a> och ge honom denna information:</p><br/>\n\n";
        echo "<p>db: $db<br><br>\n";
        echo "sql: $sql<br><br>\n";
    ...
    echo "<p>Anmälan lyckades <b>tyvärr inte</b>! Du fyllde i fel nuffror i spamskyddet. Var vänlig att gå tillbaka och försök igen.</p><br/><br/>\n\n";
    }
