...
//          $points = 100 * (1 - ($score->place - 0.5) / count($scores));
          $points = ((count($scores) + 0.5 - $score->place) * (config::$calcFixed / count($scores))) + ((pow((1 - pow((($score->place - 0.5) / count($scores)), 0.5)), config::$calcCoefficient)) * config::$calcDynamic);
