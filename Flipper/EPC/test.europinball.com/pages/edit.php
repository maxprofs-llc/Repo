            ...
            $page->startDiv('payTabs');
              ...
              if (in_array('International', config::$paymentOptions)) {
                $page->startDiv('International');
                  $page->addParagraph('Pay <span class="curCodSpans">'.config::$defaultCurrency.'</span> <span class="totlSpans">'.$costs - $person->paid.'</span> to BIC/SWIFT address <span class="bold">'.config::$swiftAddress.'</span>, IBAN number <span class="bold">'.config::$ibanAccount.'</span>.');
                $page->closeDiv();
              }
