        ...
        if ($persons && count($persons) > 0) {
          ...
          foreach ($persons as $person) {
            $json->aaData[] = array(
              ...
              $person->countryName,
              $person->getLink('ifpa'),
              (($person->getPhoto(FALSE, TRUE, FALSE)) ? '<img src="'.config::$baseHref.'/images/person.png" class="icon" title="Click to view photo"><div id="'.$person->id.'" class="photoPopup" title="'.$person->name.'"><img src="'.$person->getPhoto(FALSE, TRUE, FALSE).'"></div>' : ''),
